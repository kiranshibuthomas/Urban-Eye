    ...
    <AuthLayout isLogin={false}>
      ...
      >
        ...
        <div className="text-center">
          ...
          <h2 className="text-2xl sm:text-3xl font-bold text-primary-900 mb-2">Verify Your Email</h2>
        ...
        <div className="space-y-4">
            ...
            {otp.map((digit, index) => (
              <input
                ...
                className={`w-12 h-12 sm:w-14 sm:h-14 text-center text-lg sm:text-xl font-bold border-2 rounded-xl focus:outline-none focus:ring-2 transition-all duration-200 ${
                  digit ? 'border-accent-500 bg-accent-50' : 'border-primary-200 bg-primary-100'
          ...
          <div className="text-center">
            {timeLeft > 0 ? (
              <p className="text-sm text-primary-600">
                Code expires in <span className="font-medium text-accent-600">{formatTime(timeLeft)}</span>
              </p>
            ) : (
        ...
        <button
          ...
          className="w-full bg-gradient-to-r from-teal-600 to-teal-700 hover:from-accent-500 hover:to-accent-600 text-white font-semibold py-2.5 sm:py-3 px-4 rounded-full transition-all duration-200 flex items-center justify-center shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {isLoading ? (
            <>
              <InlineSpinner />
        ...
        <div className="text-center">
          ...
          <button
            ...
            className="text-accent-600 hover:text-accent-700 font-medium text-sm flex items-center justify-center mx-auto disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isResending ? (
              <>
                <InlineSpinner />
